            ...
            <div class="task_msg_bx" v-for="item in list" @click="goTestTaskDetail(item)">
                <div class="task_msg_left">
                    ...
                    <div class="task_man_day">
                        <p class="task_work_msg">{{item.shengyuDay}}个工作日({{item.startDay}} ~ {{item.endDay}})</p>
                        <p class="task_work_img"><img :src="item.headimage"/><span>{{item.execute_name}}</span></p>
                    </div>
                </div>
            </div>
        </div>
