          ...
          <Col span="6" v-for="(item,index) in fileData" :key="index">
            <div class="card">
              <div class="card-box" @click="fetchFileData(item.id,item.stage_file.type,item.stage_file.file,item)">
                ...
                <img  v-if="item.stage_file && item.stage_file.type == 'video'" class="card-box-pic" :src="item.stage_file.file +'?x-oss-process=video/snapshot,t_2000,f_jpg'"/>
                <img  v-if="item.stage_file && item.stage_file.type == '3d'" class="card-box-pic" :src="item.stage_file.thumb +'?x-oss-process=image/resize,l_500'"/>
                <img  v-if="item.stage_file && item.stage_file.type == 'image'" class="card-box-pic" :src="item.stage_file.file + '?x-oss-process=image/resize,l_500'"/>
                <!--<img  v-else-if="item.image[0]" class="card-box-pic" :src="item.image[item.image.length-1]"/>-->
