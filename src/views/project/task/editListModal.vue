              ...
              <div>
                <Row>
                  ...
                  <Form label-position="left" :label-width="60">
                    <FormItem label="任务名称"><Input v-model="editData.name"></Input></FormItem>
                    <FormItem label="开始时间"><DatePicker :value="editData.expect_start_date" @on-change="handleChange" format="yyyy-MM-dd" type="date" :options="disableTime" placeholder="选择时间" style="width: 100%"></DatePicker></FormItem>
                    <FormItem label="结束时间"><DatePicker :value="editData.expect_end_date" @on-change="handleChange2" format="yyyy-MM-dd" type="date" :options="disableTime2" placeholder="选择时间" style="width: 100%"></DatePicker></FormItem>
                    <FormItem label="子项目">
                      <Select @on-change='setChild' v-model="editData.project_child">
                        <Option v-for="(item,childName) in childList" :value="item.child_id" :key="item.child_id">{{item.name}}
                        </Option>
                      </Select>
                    </FormItem>
                    <FormItem label="参与人">
...
export default {
  ...
  mounted() {
    ...
    this.getTaskClass();
    this.forEachData();
    this.$bus.on("getTaskDetails",(id)=>{
      console.log("getTaskDetailsBUS");
      this.getTaskDetails(id);
    });
    this.getTaskDetails(0);
  },
  methods: {
    //获取任务列表详情
    getTaskDetails(id) {
