   ...
   <div class="real_all" v-if="isShow">
     <div class="clearfix real_header_box">
        ...
        <ul class="left-option">
          ...
          <li>
            <p :class="`option-sousuo iconfont icon-sousuo ${sousuoStyle}`" @click="getSou"></p>
            <div :class="`inputHidden ${widthNum}`">
               <Input v-model="search" placeholder="请输入搜索消息"/>
            </div>
          </li>
        </ul>
     </div>
...
export default {
  ...
  methods: {
    ...
    // getSou
    getSou () {
        const isShowBx = this.isShowBx
        if (isShowBx) {
           const searchStr = this.search
           bus.$emit('msg', searchStr)
        } else {
          this.getOptionBack(2)
        }
    },
    // 获取总数
    getAllMsg () {
    ...
    getOptionBack (index) {
      ...
      if (!index) {
        ...
        this.widthNum = 'inputClass'
        this.sousuoStyle = 'option-sousuo-father'
